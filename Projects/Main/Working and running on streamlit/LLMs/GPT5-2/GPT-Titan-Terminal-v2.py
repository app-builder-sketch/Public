...
# ==========================================
# 0. (ADDED) OPTIONAL ACCESS GATE + TV ALERTS STORAGE
# ==========================================

# NOTE: Streamlit cannot receive TradingView webhooks directly in most deployments.
# This app adds:
# - SQLite inbox to VIEW alerts
# - Parser + storage helpers
# External FastAPI receiver remains optional (see Tab 10 “Setup” panel)
...
def init_tv_db(db_path: str = TV_DB_PATH) -> None:
    try:
        ...
        # Fail-closed: do not crash app on DB issues
        pass
...
# ==========================================
st.set_page_config(layout="wide", page_title="🏦Titan Terminal-v2", page_icon="👁️")
...
# (ADDED) Optional Telegram formatting mode (non-breaking)
with st.sidebar.expander("⚙️ Telegram Advanced (Optional)", expanded=False):
    tg_parse_mode = st.selectbox("Parse Mode", ["None (Plain Text)", "MarkdownV2 (Escaped)"], index=0)
    tg_multi_hint = st.caption("Tip: You can enter multiple Chat IDs separated by commas in the Chat ID field.")
...
# (ADDED) Setup / Diagnostics panel (non-breaking)
with st.sidebar.expander("🧪 Diagnostics & Setup", expanded=False):
    ...
    st.code(TV_DB_PATH)
    st.write("**TradingView Webhook note**: Streamlit typically cannot receive webhooks directly. Use an external receiver and point it at this DB, then the app reads it.")
...
if st.session_state.get('run_analysis'):
    with st.spinner(f"Analyzing {ticker} in God Mode..."):
        ...
        if df is not None:
            ...
            # (ADDED) Ensure TV DB exists (non-breaking)
            init_tv_db()
            ...
            with tab2:
                ...
                # (ADDED) Scanner Panel (non-breaking, integrated from other builds)
                with st.expander("🧪 Multi-Asset Scanner (God Mode Score)", expanded=False):
                    st.caption("Scans a small universe using your existing indicator engine. Uses yfinance; may be slower.")
                    scan_universe = [
                    ...
                    if st.button("Run Scanner"):
                        ...
                        for sym in scan_universe[:scan_limit]:
                            sdf = safe_download(sym, "6mo" if scan_interval != "1wk" else "3y", scan_interval)
                            if sdf is None or sdf.empty:
                                rows.append({"Symbol": sym, "Status": "No Data"})
                                continue
                            try:
                                sdf = calc_fear_greed_v4(calc_indicators(sdf))
                                last_s = sdf.iloc[-1]
                                rows.append({
                                    "Symbol": sym,
            ...
            with tab10:
                ...
                if col_b1.button("Send to Telegram 🚀"):
                    if tg_token and tg_chat:
                        try:
                            ...
                            if not chat_ids:
                                st.warning("⚠️ Telegram Chat ID is empty or invalid.")
                            else:
                                # Determine parse mode
                                parse_mode = None
                                outbound_text = msg
                                if tg_parse_mode == "MarkdownV2 (Escaped)":
                                ...
                                if uploaded_file:
                                    for cid in chat_ids:
                                        okp, resp = tg_send_photo(
                                            ...
                                            caption=(tg_escape_markdown_v2(f"🔥 Analysis: {ticker}") if parse_mode == "MarkdownV2" else f"🔥 Analysis: {ticker}"),
                                ...
                                if len(outbound_text) <= max_length:
                                    for cid in chat_ids:
                                        okm, resp = tg_send_message(tg_token, cid, outbound_text, parse_mode=parse_mode)
                                else:
                                    for cid in chat_ids:
                                        part = 1
                                        for i in range(0, len(outbound_text), max_length):
                                            ...
                                            prefix = f"(Part {part}) "
                                            okm, resp = tg_send_message(tg_token, cid, prefix + chunk, parse_mode=parse_mode)
                                            part += 1
                ...
                # ==========================
                # (ADDED) TRADINGVIEW ALERTS INBOX (SQLITE)
                # ==========================
                st.markdown("---")
                with st.expander("🛰️ TradingView Alerts Inbox (SQLite)", expanded=False):
                    st.caption("Reads alerts stored in local SQLite. External webhook receiver recommended. See architecture notes. "
                               "This integrates the JSON → SQLite pattern from shared scaffold/architecture. "
                               ":contentReference[oaicite:8]{index=8} :contentReference[oaicite:9]{index=9}")

                    cols_inbox = st.columns([0.35, 0.65])
                    with cols_inbox[0]:
                        inbox_limit = st.slider("Rows", 10, 200, 50)
                        refresh_inbox = st.button("Refresh Inbox")
                    ...
                    if tv_df is None or tv_df.empty:
                        ...
                        try:
                            tv_df_display["ts_readable"] = tv_df_display["ts"].apply(lambda x: datetime.datetime.utcfromtimestamp(int(x)/1000).strftime("%Y-%m-%d %H:%M:%S UTC") if pd.notna(x) else "")
                        except Exception:
                            tv_df_display["ts_readable"] = ""

                        with cols_inbox[1]:
                            st.caption("Select a row index to broadcast that alert.")
                            selected_idx = st.number_input("Selected row index (0 = top)", min_value=0, max_value=max(0, len(tv_df_display)-1), value=0, step=1)
                        ...
                        if send_selected:
                            if not tg_token or not tg_chat:
                                ...
                                try:
                                    ...
                                    if tg_parse_mode == "MarkdownV2 (Escaped)":
                                        parse_mode = "MarkdownV2"
                                        out = tg_escape_markdown_v2(out)

                                    for cid in chat_ids:
                                        okm, resp = tg_send_message(tg_token, cid, out, parse_mode=parse_mode)
                        ...
                        with st.expander("🧾 Manual Ingest (Paste JSON) — Optional", expanded=False):
                            st.caption("If you want to test the inbox without a receiver, paste a TradingView-style JSON payload here.")
                            raw_json = st.text_area("Raw JSON", value='{"v":1,"src":"tv","symbol":"BINANCE:BTCUSDT","tf":"60","side":"BUY","confidence":65,"entry":99999.0,"stop":99000.0,"tp1":101000.0,"tp2":102000.0,"dedup_key":"BTCUSDT_60_BUY_1","reason":"Example","ts":1700000000000}', height=120)

"""
SIGNALS-MOBILE-V1
Version 19.0: Refresh + Ticker Universe FIX (Global Binance default) + Telegram Signals (Complete) + Templates + Chat Discovery + Persistent Logs

 (NO OMISSIONS)

Telegram signals scanned + missing operational features added (chat discovery via getUpdates, message preview, persistent send logs)

NOTES:
- Data source toggle: Binance Global (default) / Binance US
- TradingView exchange mapping follows selected data source (BINANCE / BINANCEUS)
- Telegram de-dup is persisted via SQLite and now scopes by (symbol, tf, chatlist, mode)
"""
...
# Optional auto-refresh (nice on mobile dashboards)
try:
...
APP_VERSION = "v19.0 (Refresh+Ticker Fix + Telegram Complete)"
...
if "quick_base" not in st.session_state:
    st.session_state.quick_base = "BTC"
if "data_source" not in st.session_state:
    st.session_state.data_source = "Binance (Global)"
if "auto_refresh_on" not in st.session_state:
    st.session_state.auto_refresh_on = False
if "auto_refresh_s" not in st.session_state:
    st.session_state.auto_refresh_s = 15
...
def set_symbol_and_refresh(base: str):
    ...
    if not base:
        base = "BTC"
    st.session_state.symbol_input = base
    hard_refresh()

def on_quick_ticker_change():
    set_symbol_and_refresh(st.session_state.quick_base)

def on_symbol_input_change():
    st.session_state.symbol_input = normalize_symbol(st.session_state.symbol_input) or "BTC"
    hard_refresh()
...
<style>
    ...
    .report-header {
        ...
        color: #ffffff;
        margin-bottom: 10px;
        border-bottom: 1px solid rgba(69,162,158,0.65);
        padding-bottom: 5px;
    }
    .report-item {
        margin-bottom: 8px;
    ...
    /* Mobile-friendly tabs/containers */
    section[data-testid="stSidebar"] { border-right: 1px solid rgba(69,162,158,0.25); }
</style>
""",
...
# CONSTANTS / SOURCES
# =============================================================================
BINANCE_GLOBAL_API_BASE = "https://api.binance.com/api/v3"
...
# =============================================================================
# SQLITE STATE (Persist Telegram de-dup + logs across reruns)
# =============================================================================
...
def db_log_signal(con, ts_utc: str, symbol: str, tf: str, trig: str, sig_key: str,
                  sent_to: str, mode: str, ok: int, detail: str, payload_preview: str):
    if con is None:
        return
    try:
        cur = con.cursor()
        cur.execute("""
            INSERT INTO signals_log(ts_utc, symbol, tf, trig, sig_key, sent_to, mode, ok, detail, payload_preview)
            VALUES(?,?,?,?,?,?,?,?,?,?)
        """, (ts_utc, symbol, tf, trig, sig_key, sent_to, mode, int(ok), (detail or "")[:500], (payload_preview or "")[:500]))
        con.commit()
    except Exception:
        pass
...
# DATA SOURCE HELPERS
# =============================================================================
def get_api_base(ds: str) -> str:
    return BINANCE_GLOBAL_API_BASE if ds == "Binance (Global)" else BINANCE_US_API_BASE

def get_tv_exchange(ds: str) -> str:
    return "BINANCE" if ds == "Binance (Global)" else "BINANCEUS"
...
def get_exchange_usdt_bases(api_base: str, nonce: int = 0) -> List[str]:
    try:
        r = requests.get(f"{api_base}/exchangeInfo", headers=HEADERS, timeout=8)
        if r.status_code != 200:
            return []
        js = r.json()
        bases = set()
        for sym in js.get("symbols", []):
...
def build_symbol_pair(base: str, quote: str = "USDT") -> str:
    base = normalize_symbol(base)
    quote = normalize_symbol(quote) or "USDT"
...
# TRADINGVIEW WIDGETS
# =============================================================================
def render_ticker_tape(selected_symbol_pair: str, tv_exchange: str, nonce: int):
    base = selected_symbol_pair.replace("USDT", "")
    ...
    symbols_json = json.dumps(
        [{"proName": f"{tv_exchange}:{b}USDT", "title": b} for b in tape_bases],
        separators=(",", ":"),
    )
...
def tv_chart_link(symbol_pair: str, tv_exchange: str) -> str:
    return f"https://www.tradingview.com/chart/?symbol={tv_exchange}:{symbol_pair}"
...
# =============================================================================
# TELEGRAM HELPERS (Complete + Missing Ops Features)
# =============================================================================
...
def tg_get_updates(token: str, limit: int = 10) -> Tuple[bool, str, List[Dict]]:
    """
    Missing feature add: discover chat IDs by reading recent updates.
    Bot must have received messages recently.
    """
    if not token:
        return False, "Missing token", []
    try:
        r = requests.get(f"https://api.telegram.org/bot{token}/getUpdates", params={"limit": int(limit)}, timeout=10)
        if r.status_code != 200:
...
def tg_send_message(
    ...
    try:
        ...
        if r.status_code != 200:
            return False, f"HTTP {r.status_code}: {r.text[:350]}"
        js = r.json()
        if js.get("ok"):
            return True, "Sent"
        return False, str(js)[:350]
    except Exception as e:
        return False, f"Error: {e}"
...
# =============================================================================
# DATA FETCH (FIX: GLOBAL BINANCE DEFAULT; BINANCEUS OPTIONAL)
# =============================================================================
@st.cache_data(ttl=8)
def get_klines(api_base: str, symbol_pair: str, interval: str, limit: int, nonce: int = 0) -> pd.DataFrame:
    try:
        r = requests.get(
            f"{api_base}/klines",
            params={"symbol": symbol_pair, "interval": interval, "limit": int(limit)},
            headers=HEADERS,
            timeout=10,
        )
        if r.status_code == 200:
        ...
        # Helpful error visibility in debug
        return pd.DataFrame()
    except Exception:
        return pd.DataFrame()
...
# =============================================================================
# TELEGRAM MESSAGE PACKETS + TEMPLATES
# =============================================================================
...
# =============================================================================
# SIDEBAR (FIXED DROPDOWN + DATA SOURCE + AUTO REFRESH)
# =============================================================================
with st.sidebar:
    ...
    st.session_state.auto_refresh_on = st.checkbox("Enable Auto Refresh", value=st.session_state.auto_refresh_on, disabled=not AUTOREFRESH_OK)
    st.session_state.auto_refresh_s = st.slider("Interval (sec)", 5, 120, int(st.session_state.auto_refresh_s), 5, disabled=(not AUTOREFRESH_OK))
    ...
    st.session_state.data_source = st.selectbox(
        ...
        index=0 if st.session_state.data_source == "Binance (Global)" else 1,
        key="data_source_select",
    )

    api_base = get_api_base(st.session_state.data_source)
    tv_exchange = get_tv_exchange(st.session_state.data_source)

    bases_all = get_exchange_usdt_bases(api_base=api_base, nonce=st.session_state.refresh_nonce)

    with st.expander("🧬 Ticker Universe (Quick Select)", expanded=True):
        if bases_all:
            list_mode = st.selectbox("List", ["Popular", "All USDT pairs"], index=0)
            if list_mode == "Popular":
                options = [b for b in POPULAR_BASES if b in bases_all]
                if not options:
                    options = bases_all[:200]
            else:
                options = bases_all

            # Search filter (mobile friendly)
            q = st.text_input("Search ticker", value="", placeholder="e.g. BTC, ETH, SOL, INJ")
            if q.strip():
                qx = q.strip().upper()
                options_f = [x for x in options if qx in x]
                options = options_f if options_f else options

            if "BTC" in options:
                default_idx = options.index("BTC")
            else:
                default_idx = 0
            ...
            st.selectbox(
                ...
                index=default_idx,
                key="quick_base",
                on_change=on_quick_ticker_change
            )
            st.caption(f"{len(bases_all)} tickers available ({st.session_state.data_source})")
        else:
            st.warning("Ticker universe unavailable. Manual input still works.")

    st.text_input("Asset (base)", value=st.session_state.symbol_input, key="symbol_input", on_change=on_symbol_input_change)
    quote_asset = st.selectbox("Quote", ["USDT"], index=0)  # keep simple + consistent

    symbol_pair = build_symbol_pair(st.session_state.symbol_input, quote_asset)

    # Validate existence (if exchangeInfo available)
    if bases_all:
        base_only = symbol_pair.replace("USDT", "")
        if base_only not in bases_all:
            st.warning(f"{base_only}USDT not found on {st.session_state.data_source}. Try Binance (Global) or pick from the list.")
    ...
    # Added: tags + templates
    tg_tags = st.text_input("Tags (optional)", value="#signals #crypto")
    tg_template = st.selectbox("Template", ["Standard", "Compact", "With Risk Line"], index=0)
        ...
        if st.button("🧪 Test Telegram", use_container_width=True, disabled=not tg_enable):
            ...
            if ok:
                ...
                if tg_chats:
                    ...
                    ok_all, res = tg_broadcast(tg_token, tg_chats, msg, mode=("MarkdownV2" if tg_format.startswith("Markdown") else "Plain"), disable_preview=True)
                    if ok_all:
                        st.success("Ping sent (all recipients).")
                    else:
                        st.error(f"Ping partial/failed: {res}")
            else:
                st.error(detail)
    with tgb:
        if st.button("🛰 Discover Chats (getUpdates)", use_container_width=True, disabled=not tg_enable):
            ok, detail, items = tg_get_updates(tg_token, limit=12)
            if ok:
                st.success("Recent updates loaded. Look for chat.id values below.")
                with st.expander("Latest getUpdates payload (trimmed)", expanded=True):
                    rows = []
                    for it in items:
                ...
                st.error(detail)
    with tgc:
        st.caption("Tip: groups/channels often look like -100xxxxxxxxxx")

# =============================================================================
# AUTO REFRESH TRIGGER
# =============================================================================
if AUTOREFRESH_OK and st.session_state.auto_refresh_on:
    # This triggers reruns; our nonce/cache design ensures fresh fetch
    st_autorefresh(interval=int(st.session_state.auto_refresh_s) * 1000, key="__autorefresh__")
...
api_base = get_api_base(st.session_state.data_source)
tv_exchange = get_tv_exchange(st.session_state.data_source)
render_ticker_tape(symbol_pair, tv_exchange, st.session_state.refresh_nonce)

# =============================================================================
# MAIN DATA
# =============================================================================
df = get_klines(api_base, symbol_pair, timeframe, limit, nonce=st.session_state.refresh_nonce)

if df.empty:
    st.error("No data returned. Try Binance (Global), check ticker, timeframe, or availability.")
    with st.expander("Debug: Request context", expanded=False):
        st.write({"api_base": api_base, "symbol_pair": symbol_pair, "timeframe": timeframe, "limit": int(limit)})
    st.stop()
...
with c_m1:
    tv_symbol = f"{tv_exchange}:{symbol_pair}"
    components.html(
        f"""
        <!-- refresh_nonce:{st.session_state.refresh_nonce} -->
        <div class="tradingview-widget-container">
...
def build_latest_alert_and_report() -> Tuple[Optional[str], Optional[str], Optional[str], Optional[str], Optional[str]]:
    """
    Returns: (alert_text, report_text, sig_key, ts_utc, trig_s)
    """
    ...
    link = tv_chart_link(symbol_pair, tv_exchange) if tg_include_chart else None
    title = f"{TG_PREFIX}: {symbol_pair} [{trig_s}]"

    # Template modes
    tags = (tg_tags or "").strip()
    if tg_template == "Compact":
        tags = tags or "#signal"
    elif tg_template == "With Risk Line":
        # We'll encode risk in the message (distance entry->stop)
        pass
    ...
    if tg_format.startswith("Markdown"):
        ...
        if tg_template == "With Risk Line":
            risk = abs(float(row["close"]) - float(row["entry_stop"])) if np.isfinite(row.get("entry_stop", np.nan)) else 0.0
            add = telegram_escape_md_v2(f"\n\nRisk: {risk:.6f} (Entry→Stop)")
            alert_text = (alert_text + add)[:4096]
    else:
        lines = [
            title,
            f"Symbol: {symbol_pair} | TF: {timeframe} | UTC: {ts}",
        ...
        if tg_template == "With Risk Line":
            risk = abs(float(row["close"]) - float(row["entry_stop"])) if np.isfinite(row.get("entry_stop", np.nan)) else 0.0
            lines += ["", f"Risk: {risk:.6f} (Entry→Stop)"]
        if tags:
            lines += ["", f"Tags: {tags}"]
        if link:
            lines += ["", f"Chart: {link}"]
    ...
    sig_key = f"{symbol_pair}|{timeframe}|{ts}|{trig_s}"
    return alert_text, report_text, sig_key, ts, trig_s

alert_text, report_text, sig_key, sig_ts, trig_s = build_latest_alert_and_report()
...
def auto_send_if_needed():
    ...
    if not tg_token or not tg_chats:
        return
    if latest_sig_idx is None or sig_key is None or alert_text is None or sig_ts is None or trig_s is None:
        return
    ...
    # Scope de-dup by chatlist + format
    chats_hash = safe_sha1(",".join(tg_chats))
    fmt = "MarkdownV2" if tg_format.startswith("Markdown") else "Plain"
    state_key = f"{symbol_pair}|{timeframe}|telegram_auto|{chats_hash}|{fmt}"
    last_sent_key = db_get_last(DB_CON, state_key)
    if last_sent_key == sig_key:
        return

    ok_all, results = tg_broadcast(
        tg_token,
        tg_chats,
        alert_text,
        mode=fmt,
    ...
    for c, ok, detail in results:
        db_log_signal(DB_CON, sig_ts, symbol_pair, timeframe, trig_s, sig_key, str(c), fmt, int(ok), detail, alert_text[:220])

    if ok_all:
        db_set_last(DB_CON, state_key, sig_key)
        st.toast("📣 Auto-sent Telegram alert", icon="✅")
    else:
        st.toast("Telegram auto-send had failures", icon="⚠️")
...
def do_send_alert():
    ...
    if not tg_chats:
        ...
        return
    if alert_text is None or sig_key is None or sig_ts is None or trig_s is None:
        st.warning("No signal event found to send.")
        return

    mode = "MarkdownV2" if tg_format.startswith("Markdown") else "Plain"
    ok_all, results = tg_broadcast(tg_token, tg_chats, alert_text, mode=mode, disable_preview=bool(tg_disable_preview))

    for c, ok, detail in results:
        db_log_signal(DB_CON, sig_ts, symbol_pair, timeframe, trig_s, sig_key, str(c), mode, int(ok), detail, alert_text[:220])

    if ok_all:
        st.success("Alert broadcast sent (all recipients).")
    else:
        st.error("Alert broadcast had failures.")
...
def do_send_report():
    ...
    ok_all, results = tg_broadcast(tg_token, tg_chats, report_text, mode="Plain", disable_preview=True)

    # Log report sends (use sig_ts if we have it; else now)
    ts = sig_ts or datetime.now(timezone.utc).isoformat().replace("+00:00", "Z")
    k = (sig_key or f"{symbol_pair}|{timeframe}|{ts}|REPORT")
    for c, ok, detail in results:
        db_log_signal(DB_CON, ts, symbol_pair, timeframe, "REPORT", k, str(c), "Plain", int(ok), detail, report_text[:220])

    if ok_all:
        st.success("Report broadcast sent (all recipients).")
    else:
        st.error("Report broadcast had failures.")
    with st.expander("Telegram send details", expanded=False):
        st.write(results)
...
with b3:
    if st.button("👁️ Preview Alert", use_container_width=True):
        if alert_text:
            st.code(alert_text)
        else:
            st.info("No alert available right now (no signal event found).")

with b4:
    if st.button("🧨 Full Reset (session)", use_container_width=True):
...
# MAIN CHART
# =============================================================================
buy_symbol = "triangle-up" if marker_style == "Triangle (legacy)" else "circle"
sell_symbol = "triangle-down" if marker_style == "Triangle (legacy)" else "circle"
...
# TABS
# =============================================================================
t1, t2, t3, t4, t5 = st.tabs(["📊 GANN", "🌊 FLOW", "🧠 SENT", "⚡ VECTOR", "📨 TG LOGS"])
...
with t2:
    ...
    f2.add_trace(go.Bar(x=df["timestamp"], y=df["money_flow"], marker_color=cols, name="Money Flow"))
...
with t5:
    st.caption("Telegram send log (persisted in SQLite). Latest 50 rows.")
    if DB_CON is not None:
        try:
            cur = DB_CON.cursor()
            cur.execute("SELECT ts_utc, symbol, tf, trig, sent_to, mode, ok, detail FROM signals_log ORDER BY id DESC LIMIT 50")
            rows = cur.fetchall()
            if rows:
                dfl = pd.DataFrame(rows, columns=["ts_utc","symbol","tf","trig","sent_to","mode","ok","detail"])
                st.dataframe(dfl, use_container_width=True)
            else:
                st.info("No telegram sends logged yet.")
        except Exception as e:
            st.error(f"Log read error: {e}")
    else:
        st.warning("SQLite unavailable in this environment.")
...
with st.expander("📜 Signal Log (latest 12)", expanded=False):
    mask = (
        (df.get("buy", False) | df.get("sell", False) |
         df.get("am_sig_buy", False) | df.get("am_sig_sell", False) |
         df.get("vec_div_bull_reg", False) | df.get("vec_div_bear_reg", False) |
         df.get("vec_div_bull_hid", False) | df.get("vec_div_bear_hid", False))
    )
    sig_df = df.loc[mask.fillna(False)].copy().tail(12)
    if sig_df.empty:
        st.caption("No signal events in window.")
    else:
        show = sig_df[[
            "timestamp","close","buy","sell","am_sig_buy","am_sig_sell",
            "vec_div_bull_reg","vec_div_bear_reg","vec_div_bull_hid","vec_div_bear_hid"
        ]].copy()
...
with st.expander("🧪 Debug (Latest Telegram Packet)", expanded=False):
    st.write({
        ...
        "refresh_nonce": st.session_state.refresh_nonce,
        "autorefresh_ok": AUTOREFRESH_OK,
        "autorefresh_on": st.session_state.auto_refresh_on,
        "autorefresh_s": st.session_state.auto_refresh_s,
    })

...
Based on Titan Intraday Pro v18.1
"""
import time
import math
import random
...
# =============================================================================
# ASSET LISTS (Dropdown Configuration)
# =============================================================================
# Grouped by sector for the dropdown functionality
ASSET_GROUPS = {
    "Majors": ["BTCUSDT", "ETHUSDT", "BNBUSDT", "SOLUSDT"],
    "Layer 1": ["ADAUSDT", "AVAXUSDT", "DOTUSDT", "NEARUSDT", "ATOMUSDT"],
    "DeFi": ["UNIUSDT", "AAVEUSDT", "LINKUSDT", "MKRUSDT"],
    "Metaverse/Gaming": ["MANAUSDT", "SANDUSDT", "AXSUSDT"],
    "Meme/Spec": ["DOGEUSDT", "SHIBUSDT", "PEPEUSDT"],
    "Custom": [] # Allows manual entry
}

# =============================================================================
# CUSTOM CSS (MOBILE OPTIMIZED & REBRANDED)
# =============================================================================
st.markdown("""
<style>
    ...
    .stButton > button {
        ...
        color: #66fcf1;
        font-weight: bold;
        height: 3.5em; /* Increased slightly for better touch */
...
def calculate_fear_greed_index(df):
    try:
        ...
        # Volatility Score
        vol_current = df['log_ret'].rolling(30).std().iloc[-1]
        vol_avg = df['log_ret'].rolling(90).std().iloc[-1]
        vol_score = 50 - ((vol_current - vol_avg) / vol_avg) * 100
...
st.title("💠 TRADING STATION MOBILE")
st.caption("v2.0 | AI TRADING ENGINE")
...
with st.sidebar:
    ...
    # --- NEW: Dropdown Asset Selection ---
    asset_category = st.selectbox("Category", list(ASSET_GROUPS.keys()), index=0)
    
    if asset_category == "Custom":
        symbol_input = st.text_input("Manual Ticker", value="BTCUSDT")
    else:
        symbol_input = st.selectbox("Asset", ASSET_GROUPS[asset_category])
        
    symbol = symbol_input.strip().upper().replace("/", "").replace("-", "")
    # Ensure it ends with USDT for Binance API compatibility
    if not symbol.endswith("USDT") and not symbol.endswith("USD"): 
         # Simple check, though manual input allows flexibility if API changes
         pass
    ...
    tg_token = st.text_input("Bot Token", value="", type="password", help="Add your Telegram Bot Token")
    tg_chat = st.text_input("Chat ID", value="", help="Add your Chat ID")
...
if not df.empty:
    ...
    with b_col1:
        if st.button("🔥 ALERT TG", use_container_width=True):
            msg = f"TRADING STATION: {symbol} | {'LONG' if last['is_bull'] else 'SHORT'} | EP: {last['close']}"
            if send_telegram_msg(tg_token, tg_chat, msg): st.success("SENT")
            else: st.error("FAIL - Check Credentials")
    ...
    sells = df[df['sell']]
    if not buys.empty: 
        fig.add_trace(go.Scatter(x=buys['timestamp'], y=buys['low'], mode='markers', marker=dict(symbol='triangle-up', size=10, color='#00ff00'), name='BUY'))
    if not sells.empty: 
        fig.add_trace(go.Scatter(x=sells['timestamp'], y=sells['high'], mode='markers', marker=dict(symbol='triangle-down', size=10, color='#ff0000'), name='SELL'))
    ...
    with t1:
        ...
        f1.add_trace(go.Scatter(x=df_g['timestamp'], y=df_g['gann_act'], mode='markers', marker=dict(color=np.where(df_g['gann_trend']==1, '#00ff00', '#ff0000'), size=3), name='Gann Pivot'))
        f1.update_layout(height=300, template='plotly_dark', margin=dict(l=0,r=0,t=0,b=0), xaxis_rangeslider_visible=False)
    ...
    st.warning(f"Unable to fetch data for {symbol}. Please check the ticker or API connectivity.")

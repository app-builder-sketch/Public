...
def check_secrets_status() -> Tuple[List[Tuple[str, str]], Dict[str, str]]:
    ...
    if secrets["telegram_token"]:
        ...
        if secrets["telegram_chat_id"]:
            ...
            status.append(
                (
                    "⚠️ TELEGRAM_CHAT_ID",
                    "Missing - Required for broadcasting (Needs user input in secrets.toml)",
                )
            )
    else:
        status.append(
            (
                "❌ TELEGRAM_BOT_TOKEN",
                "Missing - Broadcasting disabled (Needs user input in secrets.toml)",
            )
        )
    ...
    if secrets["openai_key"]:
        ...
        status.append(
            (
                "❌ OPENAI_API_KEY",
                "Missing - OpenAI disabled (Needs user input in secrets.toml)",
            )
        )
    ...
    if secrets["anthropic_key"]:
        ...
        status.append(
            (
                "❌ ANTHROPIC_API_KEY",
                "Missing - Anthropic disabled (Needs user input in secrets.toml)",
            )
        )
...
class ErrorLogger:
    ...
    def log(self, error: Exception, context: str = "") -> None:
        ...
        entry = {
            ...
            "session_id": getattr(
                st.runtime.scriptrunner.get_script_run_ctx(), "session_id", "unknown"
            )
...
def handle_error(error: Exception, user_message: str = "An error occurred") -> None:
...
class AsyncDataEngine:
    ...
    def __init__(self, max_workers: int = 10):
        ...
        self.executor = ThreadPoolExecutor(
            max_workers=max_workers, thread_name_prefix="data_fetcher"
        )
        add_script_run_ctx(self.executor)

    async def fetch_batch(
        self, assets: List[Asset], interval: str, start: datetime, end: datetime
        ...
        for asset in assets:
            task = loop.run_in_executor(
                self.executor, self._safe_fetch, asset, interval, start, end
            )
            ...
            if asset.key == "DXY":
                for alt_symbol in ["DX=F", "^DXY"]:
                    try:
                        ...
                        error_logger.log(
                            alt_err, f"DXY alt symbol fetch failed ({alt_symbol})"
                        )
            return None
...
def _cap_intraday_range(
    ...
    if (end - start).days > max_days:
        ...
        return (
            ...
            f"⚠️ Interval {interval} limited: date range capped to last {max_days} days.",
        )
    return start, end, None
    ...
    try:
        ...
        if isinstance(df.columns, pd.MultiIndex):
            if symbol in df.columns.get_level_values(0):
                ...
                df = df.copy()
                df.columns = [
                    "_".join([str(x) for x in col if x]) for col in df.columns.values
                ]
...
def fetch_fred_series(series_id: str, start: datetime, end: datetime) -> pd.DataFrame:
    ...
    try:
        ...
        out = out[
            (out.index >= pd.to_datetime(start)) & (out.index <= pd.to_datetime(end))
        ]
        out = out.dropna()
...
def load_close_series(key: str, df: pd.DataFrame) -> pd.Series:
    if df is None or df.empty:
        return pd.Series(dtype=float)
    cols = df.columns
    close_col = None
    for cand in ["Close", "Adj Close", "Adj_Close", "close"]:
        if cand in cols:
            close_col = cand
            break
    ...
    if close_col is None:
        return pd.Series(dtype=float)
    return df[close_col].rename(key)


# -----------------------------------------------------------------------------
# Indicator computation on OHLCV
# -----------------------------------------------------------------------------
def compute_overlays(df: pd.DataFrame) -> Dict[str, pd.Series]:
    ...
    try:
        ...
        if {"High", "Low", "Close"}.issubset(df.columns):
            ...
            overlays["BB Upper"] = upper
            overlays["BB Mid"] = mid
            overlays["BB Lower"] = lower

        if {"High", "Low", "Close", "Volume"}.issubset(df.columns):
            overlays["VWAP"] = vwap(df)

    except Exception as e:
        error_logger.log(e, "compute_overlays")
    return overlays
...
def compute_subpanes(df: pd.DataFrame) -> Dict[str, pd.Series]:
    ...
    try:
        ...
        sub["RSI 14"] = rsi_wilder(close, 14)
        macd_line, macd_signal, macd_hist = macd(close)
        sub["MACD"] = macd_line
        sub["MACD Signal"] = macd_signal
        sub["MACD Hist"] = macd_hist

        if {"High", "Low", "Close"}.issubset(df.columns):
            sub["ATR 14"] = atr(df["High"], df["Low"], close, 14)
    except Exception as e:
        error_logger.log(e, "compute_subpanes")
    return sub


def indicator_readout(
    df: pd.DataFrame, asset_name: str, _unused: Dict[str, Any], computed: Dict[str, pd.Series]
) -> str:
    try:
        if df is None or df.empty:
        ...
        pct = _safe_pct(last, prev)

        parts = [f"{asset_name}", f"Last: {_fmt_num(last)} ({_fmt_pct(pct)})"]

        if "RSI 14" in computed:
            rsi_val = float(computed["RSI 14"].iloc[-1])
            parts.append(f"RSI(14): {rsi_val:.1f}")

        if "MACD Hist" in computed:
            hist_val = float(computed["MACD Hist"].iloc[-1])
            parts.append(f"MACD Hist: {hist_val:+.4f}")

        if "EMA 50" in computed and "EMA 200" in computed:
            ema50 = float(computed["EMA 50"].iloc[-1])
            ema200 = float(computed["EMA 200"].iloc[-1])
            trend = "Uptrend" if ema50 > ema200 else "Downtrend"
            parts.append(f"Trend (EMA50/200): {trend}")

        return " | ".join(parts)
    except Exception as e:
        error_logger.log(e, "indicator_readout")
        return f"{asset_name} - indicator readout failed"
    ...
    try:
        ...
        if chart_type == "Candles" and {"Open", "High", "Low", "Close"}.issubset(df.columns):
            ...
            if "Close" in df.columns:
                ...
                price = df.iloc[:, 0]
            fig.add_trace(go.Scatter(x=df.index, y=price, mode="lines", name="Price"))

        # Overlays
        for name in ["EMA 50", "EMA 200", "SMA 200", "BB Upper", "BB Mid", "BB Lower", "VWAP"]:
            if name in overlays and not overlays[name].empty:
                fig.add_trace(
                    go.Scatter(
                        x=overlays[name].index,
                        y=overlays[name].values,
                        mode="lines",
                        name=name,
                        line=dict(width=1),
                    )
        ...
        fig.update_layout(
            ...
            height=480 if not show_volume else 420,
        )
    except Exception as e:
        error_logger.log(e, "make_price_figure")
    return fig


def make_volume_figure(df: pd.DataFrame, title: str) -> go.Figure:
    fig = go.Figure()
    try:
        if "Volume" not in df.columns:
            fig.update_layout(title=f"{title} - No volume data")
            return fig
        fig.add_trace(
            go.Bar(
                x=df.index,
        ...
        fig.update_layout(
            title=title, margin=dict(l=10, r=10, t=30, b=10), xaxis=dict(type="date"), height=180
        )
    except Exception as e:
        error_logger.log(e, "make_volume_figure")
    return fig


def make_rsi_figure(series: pd.Series, title: str) -> go.Figure:
    fig = go.Figure()
    try:
        fig.add_trace(go.Scatter(x=series.index, y=series.values, mode="lines", name="RSI"))
        ...
        error_logger.log(e, "make_rsi_figure")
    return fig


def make_macd_figure(
    macd_line: pd.Series, signal: pd.Series, hist: pd.Series, title: str
) -> go.Figure:
    fig = go.Figure()
    try:
        fig.add_trace(
            go.Scatter(x=macd_line.index, y=macd_line.values, mode="lines", name="MACD")
        )
        fig.add_trace(
            go.Scatter(
                x=signal.index, y=signal.values, mode="lines", name="Signal", line=dict(dash="dot")
            )
        )
        fig.add_trace(
            go.Bar(x=hist.index, y=hist.values, name="Hist", marker_color="rgba(0,0,0,0.3)")
        )
        fig.update_layout(
            title=title,
            margin=dict(l=10, r=10, t=30, b=10),
        ...
        error_logger.log(e, "make_macd_figure")
    return fig


def make_atr_figure(series: pd.Series, title: str) -> go.Figure:
    fig = go.Figure()
    try:
        fig.add_trace(go.Scatter(x=series.index, y=series.values, mode="lines", name="ATR"))
        fig.update_layout(
            title=title,
            margin=dict(l=10, r=10, t=30, b=10),
            xaxis=dict(type="date"),
        ...
        error_logger.log(e, "make_atr_figure")
    return fig
...
def default_assets() -> Dict[str, Asset]:
    ...
    assets["NDX"] = Asset("NDX", "Nasdaq 100 Index", "YF", "^NDX", "line", "US growth / duration")
    assets["DJI"] = Asset("DJI", "Dow Jones Industrial Average", "YF", "^DJI", "line", "US cyclicals")
    assets["RUT"] = Asset("RUT", "Russell 2000 Index", "YF", "^RUT", "line", "Small caps / risk")
    assets["ACWI"] = Asset("ACWI", "MSCI ACWI ETF", "YF", "ACWI", "ohlcv", "Global equities")
    assets["EEM"] = Asset("EEM", "Emerging Markets ETF", "YF", "EEM", "ohlcv", "EM equities")
    assets["QQQ"] = Asset("QQQ", "Nasdaq 100 ETF", "YF", "QQQ", "ohlcv", "Large-cap growth")
    assets["SPY"] = Asset("SPY", "S&P 500 ETF", "YF", "SPY", "ohlcv", "SPX proxy")
    ...
    assets["US13W"] = Asset("US13W", "US 13W Yield (proxy)", "YF", "^IRX", "line", "13W yield *100")
    ...
    assets["AUDUSD"] = Asset("AUDUSD", "AUD/USD", "YF", "AUDUSD=X", "line", "Commodity FX")
    assets["EURGBP"] = Asset("EURGBP", "EUR/GBP", "YF", "EURGBP=X", "line", "UK vs EU")

    # Commodities
    assets["XAU"] = Asset("XAU", "Gold (futures)", "YF", "GC=F", "ohlcv", "Gold futures")
    assets["XAG"] = Asset("XAG", "Silver (futures)", "YF", "SI=F", "ohlcv", "Silver futures")
    ...
    assets["WALCL"] = Asset(
        "WALCL", "Fed Balance Sheet (WALCL)", "FRED", "WALCL", "line", "Fed Balance Sheet"
    )
    assets["RRP"] = Asset(
        "RRP", "Reverse Repo (RRPONTSYD)", "FRED", "RRPONTSYD", "line", "ON RRP facility usage"
    )
    assets["TGA"] = Asset(
        "TGA",
    ...
    assets["SOFR"] = Asset(
        "SOFR", "SOFR (SOFR)", "FRED", "SOFR", "line", "Secured Overnight Financing Rate"
    )
...
# Data engine instance (will be re-created if max_workers changes)
if "data_engine" not in st.session_state:
    st.session_state.data_engine = AsyncDataEngine(max_workers=st.session_state.max_workers)
...
class TelegramBroadcastManager:
    ...
    def __init__(self, token: str, chat_id: str):
        ...
        if TELEGRAM_AVAILABLE and token and chat_id:
            try:
                ...
                st.warning(
                    "⚠️ Telegram bot initialization failed. Broadcasting disabled. "
                    "Check TELEGRAM_BOT_TOKEN / TELEGRAM_CHAT_ID."
                )
                self.bot = None

    async def broadcast_message(
        self, message: str, parse_mode: str = "HTML", silent: bool = False
    ) -> bool:
    ...
    def queue_message(self, message: str, priority: int = 0) -> None:
    ...
    def format_signal_message(
        self, asset: str, signal: Dict[str, Any], ai_analysis: str = ""
    ) -> str:
        ...
        if "macd" in signal:
            message += (
                f"📊 <b>MACD:</b> {signal['macd']['histogram']:+.4f} "
                f"({signal['macd']['signal']})\n"
            )
        ...
        message += (
            "\n⚠️ <i>Risk Warning: This is not financial advice. "
            "Always do your own research.</i>"
        )
        return message


broadcast_manager: Optional[TelegramBroadcastManager] = None
...
class AIAnalyzer:
    ...
    def generate_prompt(
        ...
        focus_areas = focus_areas or ["Price Action", "Trend Analysis"]
        focus_text = ", ".join(focus_areas)
        depth_text = f"Depth level: {depth} (1-5 scale)."
...
Provide concise, actionable analysis in 4-6 sentences. Use professional trader language.
"""
        return prompt
    ...
    async def analyze(
        ...
        signal_data = {
            ...
            "change_1d": _safe_pct(close_series.iloc[-1], close_series.iloc[-2])
            if len(close_series) >= 2
            else 0,
            "change_1w": _safe_pct(close_series.iloc[-1], close_series.iloc[-8])
            if len(close_series) >= 8
            else 0,
        }

        prompt = self.generate_prompt(asset_name, df, computed, signal_data, depth, focus_areas)
        ...
        try:
            if provider == "openai" and self.openai_client:
                ...
                return (
                    "AI analysis unavailable: No valid API key configured. "
                    "Needs user input in secrets.toml."
                )
        except Exception as e:
...
class LiveDataManager:
    ...
    async def get_live_quotes_async(self, keys: List[str]) -> Dict[str, Dict[str, Any]]:
        quotes: Dict[str, Dict[str, Any]] = {}
        keys = keys[:10]

        assets = [get_asset(k) for k in keys]
        ...
        try:
            results = await st.session_state.data_engine.fetch_batch(
                assets, "1m", start, end
            )
            for k in keys:
                asset = get_asset(k)
                if not asset:
                    continue
                df = results.get(asset.key)
                if df is None or df.empty:
                    continue
                close = load_close_series(asset.key, df)
                if close.empty:
                    continue
                price = close.iloc[-1]
                change = (
                    _safe_pct(close.iloc[-1], close.iloc[-2]) if len(close) >= 2 else 0.0
                )
                quotes[k] = {"price": price, "change": change, "asset": asset}
        except Exception as e:
            error_logger.log(e, "Live quotes batch")

        return quotes


live_manager = LiveDataManager(refresh_interval=st.session_state.live_refresh_interval)
...
# Secrets health badge in header
secrets_ok = all(
    s.startswith("✅")
    for s, _ in status
    if "API_KEY" in s or "TELEGRAM_BOT_TOKEN" in s or "TELEGRAM_CHAT_ID" in s
)
health_label = "✅ Secrets OK" if secrets_ok else "⚠️ Secrets Incomplete"
...
if secrets["telegram_token"] and secrets["telegram_chat_id"]:
    if st.sidebar.button("🔗 Connect Telegram Bot"):
        try:
            ...
            st.session_state.telegram_connected = broadcast_manager.bot is not None
    ...
    st.sidebar.warning(
        "⚠️ Telegram secrets missing. Broadcasting disabled. Needs user input in secrets.toml."
    )
...
with st.sidebar.expander("🤖 AI Configuration", expanded=True):
    ...
    if providers:
        ...
        st.caption("AI analysis active. Token usage and latency depend on provider.")
    else:
        st.info(
            "AI analysis: **Needs user input**. "
            "Add OPENAI_API_KEY or ANTHROPIC_API_KEY to secrets.toml."
        )

# Live Data Controls + Auto-refresh
with st.sidebar.expander("⚡ Live Data & Refresh", expanded=True):
    st.session_state.live_refresh_interval = st.slider(
        ...
        step=30,
        help="Controls banner and auto-refresh cadence.",
    )
    live_manager.refresh_interval = st.session_state.live_refresh_interval

    st.session_state.auto_refresh = st.checkbox(
        "Enable Auto Refresh (experimental)",
        value=st.session_state.auto_refresh,
        help="Safest default is OFF. When ON, page re-runs on live interval.",
    )
...
with st.sidebar.expander("🚀 Performance Mode", expanded=True):
    mode = st.selectbox(
        "Performance Profile",
        ["Safe", "Balanced", "Aggressive"],
        index=["Safe", "Balanced", "Aggressive"].index(st.session_state.performance_mode),
        help=(
            "Safe: best stability for few assets. "
            "Balanced: moderate concurrency. "
            "Aggressive: 500+ tickers, may stress data providers."
        ),
    )
    st.session_state.performance_mode = mode

    # Map mode to defaults
    if mode == "Safe":
        suggested_workers = 8
        max_assets_hint = 50
    elif mode == "Balanced":
    ...
    st.session_state.max_workers = st.slider(
        ...
        step=2,
        help="Controls AsyncDataEngine concurrency.",
    )
    st.caption(
        f"Guidance: This mode is tuned for up to ~{max_assets_hint} assets per view. "
        "For more, reduce selection or increase interval."
    )

    # Recreate data engine if worker count changed
    if (
        "data_engine" not in st.session_state
        or st.session_state.data_engine.max_workers != st.session_state.max_workers
    ):
        st.session_state.data_engine = AsyncDataEngine(max_workers=st.session_state.max_workers)
        st.info("Data engine reconfigured with new worker count.")
...
if st.sidebar.toggle("📊 Data Selection", value=st.session_state.ui_expanded["data_selection"]):
    ...
    keys_in_cat = [
        k for k in st.session_state.custom_watchlists[category] if get_asset(k) is not None
    ]
    selected_keys = st.multiselect(
        "Assets",
        keys_in_cat,
        default=keys_in_cat[: min(5, len(keys_in_cat))],
        help="Select multiple assets to analyze. For 500+ tickers, use fewer per view.",
    )
else:
...
if st.sidebar.toggle("⏰ Timeframe", value=st.session_state.ui_expanded["timeframe"]):
    ...
    interval = st.selectbox(
        ...
        index=0,
        help="Intraday intervals are range-limited for performance.",
    )
    ...
    dr = st.date_input(
        ...
        help="Select analysis period. Intraday ranges may be capped.",
    )
...
if st.sidebar.toggle("📈 Chart Settings", value=st.session_state.ui_expanded["chart_indicators"]):
    ...
    chart_type = "Candles"
    show_volume = True
    use_ema50 = use_ema200 = True
    use_sma200 = use_vwap = use_bb = False
    show_rsi = show_macd = True
    show_atr = False

# Ratio mode (kept in main body for clarity)
use_ratio_mode = st.sidebar.toggle("Ratio Mode (A/B)", value=False)
ratio_preset_name = None
ratio_a = None
ratio_b = None

if use_ratio_mode:
    ratio_preset_name = st.sidebar.selectbox(
        "Ratio Preset",
...
st.caption(
    f"{health_label} • Professional macro analysis with LLM insights and Telegram broadcasting"
)

# Live Ticker Banner (batch)
banner_placeholder = st.empty()
if selected_keys:
    try:
        if live_manager.should_refresh():
            quotes = asyncio.run(live_manager.get_live_quotes_async(selected_keys[:8]))
        else:
            quotes = {}
        if quotes:
            with banner_placeholder.container():
                cols = st.columns(len(quotes))
                for i, (key, quote) in enumerate(quotes.items()):
                    with cols[i]:
                        st.metric(
                            label=key,
        ...
        handle_error(e, "Live banner update failed")
...
with tab_chart:
        ...
        if use_ratio_mode:
            ...
            if not a_asset or not b_asset:
                ...
                with st.spinner("Fetching ratio data..."):
                    try:
                        st.session_state.current_context = f"Ratio: {ratio_a}/{ratio_b}"
                        df_map = asyncio.run(
                            st.session_state.data_engine.fetch_batch(
                                [a_asset, b_asset], yf_interval, start_dt, end_dt
                            )
                        )
                        df_a = df_map.get(a_asset.key)
                        df_b = df_map.get(b_asset.key)
                        if df_a is None or df_b is None:
                            st.error("No data returned for ratio assets")
                        else:
                            ...
                            overlays = {
                                ...
                                if (k == "EMA 50" and use_ema50)
                                or (k == "EMA 200" and use_ema200)
                                or (k == "SMA 200" and use_sma200)
                                or (k == "VWAP" and use_vwap)
                                or (k.startswith("BB") and use_bb)
                            }

                            fig = make_price_figure(df_ratio, title, "Line", overlays, False)
                            st.plotly_chart(fig, use_container_width=True, key="ratio_chart")

                            if "RSI 14" in sub and show_rsi:
                                st.plotly_chart(
                                    make_rsi_figure(sub["RSI 14"], "RSI 14"),
                                    use_container_width=True,
                                )

                            if (
                                show_macd
                                and {"MACD", "MACD Signal", "MACD Hist"}.issubset(sub.keys())
                            ):
                                st.plotly_chart(
                                    make_macd_figure(
                                        sub["MACD"],
                            ...
                            if show_atr and "ATR 14" in sub:
                                st.plotly_chart(
                                    make_atr_figure(sub["ATR 14"], "ATR 14"),
                                    use_container_width=True,
                                )

                            if (
                                hasattr(st.session_state, "ai_analyzer")
                                and st.button("🤖 Analyze Ratio", key="ai_ratio")
                            ):
                                with st.spinner("AI analyzing ratio..."):
                                    ai_result = asyncio.run(
                                        st.session_state.ai_analyzer.analyze(
                                            ...
                                            depth=3,
                                            focus_areas=["Price Action", "Trend Analysis"],
                                        )
                                    )
                                with right:
            ...
            if not selected_keys:
                st.info("Select assets to chart in the sidebar Data Selection section.")
            else:
                asset_key = st.selectbox(
                    "Active Chart", selected_keys, index=0, key="chart_selector"
                )
                ...
                if asset:
                    with st.spinner("Fetching market data..."):
                        try:
                            ...
                            df_map = asyncio.run(
                                st.session_state.data_engine.fetch_batch(
                                    [asset], yf_interval, start_dt, end_dt
                                )
                            )
                            df = df_map.get(asset.key)
                            if df is None or df.empty:
                                st.error("No data available for selected asset.")
                            else:
    ...
    with right:
        ...
        readout_key = st.selectbox(
            "Asset", selected_keys if selected_keys else ["SPX"], key="readout_asset"
        )
        ...
        if readout_asset:
            try:
                df_map = asyncio.run(
                    st.session_state.data_engine.fetch_batch(
                        [readout_asset], yf_interval, start_dt, end_dt
                    )
                )
                df = df_map.get(readout_asset.key)
                if df is None or df.empty:
                    st.info("No data for indicator readout.")
                else:
                    overlays = compute_overlays(df)
                    sub = compute_subpanes(df)
                    ...
                    if st.session_state.telegram_connected and st.button(
                        "📡 Broadcast Readout"
                    ):
                        message = f"📊 <b>Technical Readout - {readout_asset.key}</b>\n\n{txt}"
                        asyncio.run(
                            st.session_state.broadcast_manager.broadcast_message(message)
                        )
...
with tab_dashboard:
    ...
    dash_keys = st.multiselect(
        ...
        default=st.session_state.custom_watchlists.get("Core Macro (must-follow)", [])[:20],
        key="dash_selector",
        help=(
            "Select assets for the overview table. "
            "For best performance with 500+ assets in total universe, limit selection per view."
        ),
    )
    ...
    if dash_keys:
        with st.spinner("Building dashboard..."):
            assets = [get_asset(k) for k in dash_keys]
            assets = [a for a in assets if a is not None]

            try:
                df_map = asyncio.run(
                    st.session_state.data_engine.fetch_batch(
                        assets, dash_yf_interval, dash_start, dash_end
                    )
                )
                for k in dash_keys:
                    asset = get_asset(k)
                    ...
                    c_1w = (
                        float(close.iloc[-(w_back + 1)])
                        if len(close) >= (w_back + 1)
                        else np.nan
                    )
                    c_1m = (
                        float(close.iloc[-(m_back + 1)])
                        if len(close) >= (m_back + 1)
                        else np.nan
                    )
                    c_3m = (
                        float(close.iloc[-(q_back + 1)])
                        if len(close) >= (q_back + 1)
                        else np.nan
                    )
                ...
                handle_error(e, "Dashboard batch fetch")
    ...
    if rows:
        ...
        if st.session_state.telegram_connected and st.button("📡 Broadcast Dashboard"):
            ...
            for _, row in df_dash.iterrows():
                ...
                message += (
                    f"Last: {_fmt_num(row['Last'])} | 24h: {_fmt_pct(row['24h'])}\n\n"
                )

            asyncio.run(st.session_state.broadcast_manager.broadcast_message(message))
            st.success("✅ Dashboard broadcast sent!")
    else:
        st.info("No dashboard data available. Adjust selection or interval.")

# -----------------------------------------------------------------------------
# CORRELATION TAB (Batch fetching)
# -----------------------------------------------------------------------------
with tab_correlation:
    ...
    corr_keys = st.multiselect(
        ...
        key="corr_selector",
        help="Select assets for correlation matrix. Large selections increase compute time.",
    )
    ...
    if len(corr_keys) < 2:
        st.info("Select at least 2 assets for correlation.")
    else:
        with st.spinner("Computing correlation..."):
            assets = [get_asset(k) for k in corr_keys]
            assets = [a for a in assets if a is not None]

            try:
                df_map = asyncio.run(
                    st.session_state.data_engine.fetch_batch(
                        assets, yf_interval, start_dt, end_dt
                    )
                )
                ...
                if len(series_list) >= 2:
                    ...
                    if aligned.shape[0] < corr_min_obs:
                        st.warning(
                            f"Only {aligned.shape[0]} observations. Expand date range or reduce min observations."
                        )
                    ...
                    if st.session_state.telegram_connected and st.button(
                        ...
                        message += "\n".join(high_corr[:10]) if high_corr else "None.\n"
                        asyncio.run(
                            st.session_state.broadcast_manager.broadcast_message(message)
                        )
...
with tab_telegram:
    ...
    if not st.session_state.telegram_connected:
        st.warning(
            "⚠️ Telegram not connected. Add credentials to secrets.toml and connect from sidebar."
        )
        st.info(
            "**Setup Instructions:**\n"
            ...
            "5. Add TELEGRAM_BOT_TOKEN and TELEGRAM_CHAT_ID to secrets.toml"
        )
    else:
        ...
        if st.button("🚀 Send Broadcast"):
            if quick_msg.strip():
                asyncio.run(
                    st.session_state.broadcast_manager.broadcast_message(quick_msg)
                )
        ...
        sig_asset = st.selectbox(
            "Signal Asset", selected_keys if selected_keys else list(ASSETS.keys())
        )
        sig_type = st.selectbox(
            "Signal Type", ["Bullish", "Bearish", "Neutral", "Breakout", "Reversal"]
        )
        sig_entry = st.number_input(
            "Entry Price",
            value=0.0,
            format="%.4f",
            help="Leave 0 for market. Needs user input if applying risk templates.",
        )
        sig_stop = st.number_input("Stop Loss", value=0.0, format="%.4f")
        sig_target = st.number_input("Target Price", value=0.0, format="%.4f")
        sig_rationale = st.text_area(
            "Rationale", placeholder="Technical/fundamental reasoning..."
        )

        if st.button("📡 Broadcast Signal"):
            try:
                asset = get_asset(sig_asset)
                st.session_state.current_context = f"Signal: {sig_asset}"
                df_map = asyncio.run(
                    st.session_state.data_engine.fetch_batch(
                        [asset], yf_interval, start_dt, end_dt
                    )
                ...
                df = df_map.get(asset.key)
                if df is None or df.empty:
                    st.error("No data for selected signal asset.")
                else:
                    close = load_close_series(sig_asset, df)
                    signal_data = {
                        "price": close.iloc[-1] if not close.empty else 0,
                        "change_1d": _safe_pct(close.iloc[-1], close.iloc[-2])
                        if len(close) >= 2
                        else 0,
                    ...
                    ai_analysis = (
                        ...
                        if hasattr(st.session_state, "ai_analyzer")
                    ...
                    if sig_entry > 0:
                        message += (
                            f"\n<b>Entry:</b> {_fmt_num(sig_entry)} | "
                            f"<b>Stop:</b> {_fmt_num(sig_stop)} | "
                            f"<b>Target:</b> {_fmt_num(sig_target)}"
                        )
        ...
        st.markdown("### 🧾 Broadcast History (Session)")
        # Minimal history from error_log-style storage is not implemented; placeholder.
...
with tab_ai:
    ...
    if not hasattr(st.session_state, "ai_analyzer"):
        st.warning(
            "⚠️ AI not configured. AI analysis: Needs user input. "
            "Add OPENAI_API_KEY or ANTHROPIC_API_KEY to secrets.toml."
        )
    else:
        ai_asset = st.selectbox(
            "AI Analysis Asset", selected_keys if selected_keys else list(ASSETS.keys())
        )
        ...
        analysis_depth = st.slider(
            ...
            5,
            3,
            help="Higher = more detailed but slower and more tokens used.",
        )
                ...
                with st.spinner(f"Running {analysis_depth}x AI analysis..."):
                    ...
                    if df is None or df.empty:
                        ...
                        if "RSI 14" in sub:
                            ...
                            validation.append(
                                f"RSI: {rsi_val:.1f} - {'Valid' if 0 < rsi_val < 100 else 'Out of range'}"
                            )
        ...
        if (
            hasattr(st.session_state, "last_ai_analysis")
            and st.session_state.telegram_connected
            and st.button("📡 Broadcast AI Analysis")
        ):
            message = (
                f"🤖 <b>AI Analysis - {ai_asset}</b>\n\n{st.session_state.last_ai_analysis}"
            )
            asyncio.run(
                st.session_state.broadcast_manager.broadcast_message(message)
            )
...
# -----------------------------------------------------------------------------
# TELEGRAM BOT SETUP (optional / experimental)
# -----------------------------------------------------------------------------
async def setup_telegram_bot() -> None:
    """Initialize Telegram bot command handlers (experimental)."""
    if not st.session_state.telegram_connected or not TELEGRAM_AVAILABLE:
        return
    ...
    async def status(update, context):
        await update.message.reply_text(
            ...
            f"📊 Active assets (current view): {len(selected_keys)}"
        )
    ...
    # In production, run polling/webhook in separate process/service.
...
with tab_settings:
    ...
    sys_info = {
        ...
        "Max Workers": st.session_state.max_workers,
        "Performance Mode": st.session_state.performance_mode,
        "Error Log Size": len(st.session_state.error_log),
    }
    st.json(sys_info)

    st.markdown("### 📌 Capability Notes")
    st.markdown(
        "- Designed to handle 500+ tickers total universe via caching and batch fetch.\n"
        "- For best UX, limit per-view selection (dashboard/correlation) according to performance mode.\n"
        "- AI and Telegram features require secrets; missing items are marked as 'Needs user input'."
    )
...
# -----------------------------------------------------------------------------
# BACKGROUND TASKS (Auto-refresh, Telegram)
# -----------------------------------------------------------------------------
if st.session_state.telegram_connected and hasattr(st.session_state, "broadcast_manager"):
    try:
        asyncio.run(st.session_state.broadcast_manager.process_queue())
    except Exception as e:
        error_logger.log(e, "Broadcast queue processing")

if TELEGRAM_AVAILABLE and "telegram_bot_setup" not in st.session_state:
    try:
        # Mark as experimental; running bots inside Streamlit is not recommended in prod
        asyncio.run(setup_telegram_bot())
        st.session_state.telegram_bot_setup = True
    except Exception as e:
        error_logger.log(e, "Telegram bot setup")

if st.session_state.auto_refresh and live_manager.should_refresh():

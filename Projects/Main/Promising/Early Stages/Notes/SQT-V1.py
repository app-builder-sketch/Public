...
class HZQEOEngine:
    ...
    def calculate_zeta_component(self, df: pd.DataFrame) -> pd.Series:
        ...
        for idx in range(self.lookback, len(df)):
            ...
            # Ensure we have enough data for volatility calculation (20 periods)
            if idx >= 20:
    ...
    def calculate(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        with st.spinner("Computing Hyperbolic Zeta Components..."):
            ...
            results['tunnel_prob'] = 1.0 - (roll_max - df['close']) / denom
...
# ==========================================
# 4. DATA INFRASTRUCTURE (Yahoo Finance)
# ==========================================
@st.cache_data(ttl=60)
def fetch_market_data(symbol, period, interval):
    """
    Fetches market data using yfinance (Free API).
    """
    try:
        # yfinance requires specific format. 
        # Map generic 'period' request to yfinance valid periods if necessary
        # Here we trust the UI input
        
        # NOTE: yfinance interval constraints:
        # 1m data is only available for 7 days
        # 5m, 15m, 30m for 60 days
        # 1h for 730 days
        ...
        # yfinance uses Capitalized columns, let's standardize to lowercase
        df.reset_index(inplace=True)
        df.columns = [c.lower() for c in df.columns]
        
        # Rename 'date' or 'datetime' to 'timestamp' for consistency with downstream code
        if 'date' in df.columns:
            df.rename(columns={'date': 'timestamp'}, inplace=True)
        elif 'datetime' in df.columns:
            df.rename(columns={'datetime': 'timestamp'}, inplace=True)
            
        # Ensure timestamp is tz-naive or standardized (remove timezone if present for easier plotting)
...
def init_session_state():
    defaults = {
        "sym": "BTC-USD",  # Default adjusted for YFinance
        "tf_period": "5d", # Default period
        "tf_interval": "15m", # Default interval
...
with st.sidebar:
    ...
    with st.expander("🌍 Market Feed (Yahoo Free)", expanded=True):
        st.session_state.sym = st.text_input("Asset Ticker (e.g., AAPL, BTC-USD, NVDA)", st.session_state.sym)
        
        c1, c2 = st.columns(2)
        with c1:
            st.session_state.tf_interval = st.selectbox("Interval", ["1m","2m","5m","15m","30m","1h","1d"], index=3)
        with c2:
            st.session_state.tf_period = st.selectbox("Lookback", ["1d","5d","1mo","3mo","6mo","1y","2y","5y"], index=1)
...
# ==========================================
# Validating inputs before fetch
if not st.session_state.sym:
    st.warning("Please enter a valid ticker symbol.")
    st.stop()

df = fetch_market_data(st.session_state.sym, st.session_state.tf_period, st.session_state.tf_interval)

if df.empty:
    st.warning(f"⚠️ No Data Stream Available for {st.session_state.sym}. Verify Ticker or adjust Interval/Period.")
    st.info("Note: 1m data is limited to 7 days lookback on Yahoo Finance.")
    st.stop()
...
# Ensure we have enough data to prevent index errors
if len(df) < 2:
...
# Price formatting logic based on asset value size
price_fmt = "{:.2f}"
if last['close'] < 1.0: price_fmt = "{:.4f}"
if last['close'] < 0.01: price_fmt = "{:.6f}"

render_metric(col1, "ASSET PRICE", price_fmt.format(last['close']))
...
with t1:
    ...
    with col_l:
        ...
        fig.add_hline(y=0.8, line_dash="dot", line_color="#FF1744", row=2, col=1, annotation_text="Overbought")
        fig.add_hline(y=-0.8, line_dash="dot", line_color="#00E676", row=2, col=1, annotation_text="Oversold")
        ...
        # Remove weekends/gaps if desired (Common in crypto to keep, but for stocks removing weekends is good)
        # Using simple category axis often better for financial data if gaps are issue, but keeping date here.
...
with t2:
    ...
    fig.add_trace(go.Candlestick(
        x=df['timestamp'], open=df['open'], high=df['high'], low=df['low'], close=df['close'], 
        name='Price',
        increasing_line_color='#212121', decreasing_line_color='#212121',
        increasing_fillcolor='#212121', decreasing_fillcolor='#212121'
    ))
    fig.add_trace(go.Scatter(x=df['timestamp'], y=df['br_u'], line=dict(color='rgba(0, 230, 118, 0.8)'), name='Neural Upper'))
    fig.add_trace(go.Scatter(x=df['timestamp'], y=df['br_l_band'], line=dict(color='rgba(255, 23, 68, 0.8)'), fill='tonexty', fillcolor='rgba(255,255,255,0.05)', name='Neural Lower'))
    ...
    fig.add_trace(go.Scatter(
        x=buy_sig['timestamp'], y=buy_sig['low'] * 0.999, 
        mode='markers+text', marker=dict(symbol='triangle-up', size=14, color='#00E676'), 
        text=["BUY"]*len(buy_sig), textposition="bottom center", name='Neural Buy'
    ))
    fig.add_trace(go.Scatter(
        x=sell_sig['timestamp'], y=sell_sig['high'] * 1.001, 
        mode='markers+text', marker=dict(symbol='triangle-down', size=14, color='#FF1744'),
        text=["SELL"]*len(sell_sig), textposition="top center", name='Neural Sell'
    ))
    
    fig.update_layout(
        template="plotly_dark", paper_bgcolor="#151925", plot_bgcolor="#151925", 
        height=600, margin=dict(l=10,r=10,t=10,b=10),
        xaxis_rangeslider_visible=False,
        title="Adaptive Neural Bands & Hysteresis Signals"
    )
    st.plotly_chart(fig, use_container_width=True)
...
with t3:
    ...
    fig.update_layout(template="plotly_dark", paper_bgcolor="#151925", plot_bgcolor="#151925", height=600, xaxis_rangeslider_visible=False)
    st.plotly_chart(fig, use_container_width=True)

# TAB 4: MARKET STRUCTURE (SMC)
with t4:
    fig = go.Figure()
    
    # Simple line for Close
    fig.add_trace(go.Scatter(x=df['timestamp'], y=df['close'], line=dict(color='#888', width=1), name='Price'))
    
    # SMC Base Line
    fig.add_trace(go.Scatter(x=df['timestamp'], y=df['smc_base'], line=dict(color='#00B0FF', width=2), name='Institutional Baseline'))
    
    # Fair Value Gaps
    # Highlight bars with gaps
    fvg_bull = df[df['fvg_bull']]
    fvg_bear = df[df['fvg_bear']]
    
    fig.add_trace(go.Scatter(
        x=fvg_bull['timestamp'], y=fvg_bull['low'], 
        mode='markers', marker=dict(symbol='square', color='rgba(0, 230, 118, 0.5)', size=6),
        name='FVG Bull'
    ))
    
    fig.add_trace(go.Scatter(
        x=fvg_bear['timestamp'], y=fvg_bear['high'], 
        mode='markers', marker=dict(symbol='square', color='rgba(255, 23, 68, 0.5)', size=6),
        name='FVG Bear'
    ))
    
    fig.update_layout(
        template="plotly_dark", paper_bgcolor="#151925", plot_bgcolor="#151925", height=600,
        title="Smart Money Concepts: FVGs & Baseline Structure"
    )
    st.plotly_chart(fig, use_container_width=True)
